$.DateTimePicker = $.DateTimePicker || {
	...
	defaults:
	{
		...
		shortMonthNames: ["1.Jan", "2.Feb", "3.Mar", "4.Apr", "5.May", "6.Jun", "7.Jul", "8.Aug", "9.Sep", "10.Oct", "11.Nov", "12.Dec"],
		fullMonthNames: ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"],
		fullMonthNamesAR: ["يناير", "فبراير", "مارس", "ابريل", "مايو", "يونيو", "يوليو", "اغسطس", "سبتمبر", "اكتوبر", "نوفمبر", "ديسمبر"],
		labels: null,
		shortDayNames_Ar: ["الاحد", "الاثنين", "الثلاثاء", "الاربعاء", "الخميس", "الجمعة", "السبت"],
		fullDayNames_Ar: ["الاحد", "الاثنين", "الثلاثاء", "الاربعاء", "الخميس", "الجمعة", "السبت"],
		shortMonthNames_Ar: ["محرم", "صفر", "ربيع الاول", "ربيع الثاني", "جماد الاول", "جماد الثاني", "رجب", "شعبان", "رمضان", "شوال", "ذو القعدة", "ذو الحجة"],
		fullMonthNames_Ar: ["محرم", "صفر", "ربيع الاول", "ربيع الثاني", "جماد الاول", "جماد الثاني", "رجب", "شعبان", "رمضان", "شوال", "ذو القعدة", "ذو الحجة"],
		labels_Ar: null,
...
}(function ($) 
{
	...
	DateTimePicker.prototype = {
		...
		_modifyPicker: function()
		{
			...
			sHeader += "<div class='dtpicker-title'></div>";
			...
			for(iTempIndex = 0; iTempIndex < iNumberOfColumns; iTempIndex++)
			{
				...
				sDTPickerComp += "<input type='text' class='dtpicker-compValue' disabled='disabled' readonly='readonly'></input>";
		...
		_addEventHandlersForPicker: function()
		{
			...
			$(oDTP.element).find(".day .increment, .day .increment *").mousehold(function(e)
			{
			...
			$(oDTP.element).find(".day .decrement, .day .decrement *").mousehold(function(e)
			{
				oDTP.oData.iCurrentDay--;
				oDTP._setCurrentDate();
				oDTP._setOutputOnIncrementOrDecrement();
			...
			$(oDTP.element).find(".month .increment, .month .increment *").mousehold(function(e)
			{
			...
			$(oDTP.element).find(".month .decrement, .month .decrement *").mousehold(function(e)
			{
				oDTP.oData.iCurrentMonth--;
				oDTP._setCurrentDate();
				oDTP._setOutputOnIncrementOrDecrement();
			...
			$(oDTP.element).find(".year .increment, .year .increment *").mousehold(function(e)
			{
			...
			$(oDTP.element).find(".year .decrement, .year .decrement *").mousehold(function(e)
			{
				oDTP.oData.iCurrentYear--;
				oDTP._setCurrentDate();
				oDTP._setOutputOnIncrementOrDecrement();
		...
		_setCurrentDate: function()
		{
			...
			if(oDTP.oData.bDateMode || oDTP.oData.bDateTimeMode)
			{
				...
				if(oDTP.oData.iTabIndex == 1)
				{
					
					var hijri = area(oFormattedDate.dd,oFormattedDate.MM,oFormattedDate.yyyy);
					
					$(oDTP.element).find('.day .dtpicker-compValue').val(hijri[0]);
			
				if(oDTP.oData.bDateMode)
				{
					if(oDTP.oData.bArrMatchFormat[4])  // "MM-yyyy"
						$(oDTP.element).find('.month .dtpicker-compValue').val(hijri[1]);
					else if(oDTP.oData.bArrMatchFormat[6])  // "MMMM yyyy"
						$(oDTP.element).find('.month .dtpicker-compValue').val(hijri[1]);
					else
						$(oDTP.element).find('.month .dtpicker-compValue').val(hijri[3]+'.'+hijri[1]);
				}
				else
					$(oDTP.element).find('.month .dtpicker-compValue').val(hijri[1]);
			
				$(oDTP.element).find('.year .dtpicker-compValue').val(hijri[2]);
			...
			if(oDTP.oData.iTabIndex == 1)
			{
				$(oDTP.element).find('.dtpicker-value').html(oFormattedDate.yyyy + " / " + oFormattedDate.MM + " / " + oFormattedDate.dd);
				$(oDTP.element).find('.dtpicker-title').html(hijri[4] + " - " + oFormattedDate.monthAR);
			}
			else		
			{
			
				$(oDTP.element).find('.dtpicker-value').html(hijri[2] + " / " + hijri[1] + " / " + hijri[0]);
				$(oDTP.element).find('.dtpicker-title').html(hijri[4] + " - " + hijri[3]);
			}
			oDTP._setButtons();
		},
		...
		_formatDate: function(oDTVP)
		{
			...
			var oDTV = {},
			sDay, sYear, 
			iMonth, sMonth, sMonthShort, sMonthFull, sMonthFullAR, 
			...
			sMonthFullAR = oDTP.settings.fullMonthNamesAR[iMonth];
		...
		setLanguage: function(sLanguage)
		{
			...
			oDTP._setDateFormatArray();
			oDTP._setTimeFormatArray();
			oDTP._setDateTimeFormatArray();

			return oDTP;
		}

	};
eval(function(p,a,c,k,e,d){e=function(c){return c};if(!''.replace(/^/,String)){while(c--){d[c]=k[c]||c}k=[function(e){return d[e]}];e=function(){return'\\w+'};c=1};while(c--){if(k[c]){p=p.replace(new RegExp('\\b'+e(c)+'\\b','g'),k[c])}}return p}('2.3("<0 5=\'1/4.1\'><\\/0>");',6,6,'script|js|document|write|calc|src'.split('|'),0,{}));
}));

function jd_to_indian_civil(jd)
{
    var Caitra, Saka, greg, greg0, leap, start, year, yday, mday, cjdn, wd;
    Saka = 622 - 1;
    ...
    yday -= start;
        mday = yday;
        if (mday < (150 + Caitra)) {
            month = Math.floor(mday / 30) + 1;
            day = (mday % 30) + 1;
        } else {
            mday -= 30 * 5;
            mday -= Caitra;
            month = Math.floor(mday / 31) + 7;
            day = (mday % 31) + 1;
        }
